...
function setPage(index)
  ...
  local i
  for i=start,stop do
    local process=ceshare.FullProcessListView[i].process
    local title=ceshare.FullProcessListView[i].title
    
    
    if process and title then
      local s=process..' - '..title      
      ceshare.AllTableForm.List.items.add(s)
    end
