...
#else
int MDC_ServerPipe=0xcececece; //
#endif

int MDC_ShuttingDown = 0;
int attachcount = 0;
...
std::map<void *, int> attachedthreads;


std::vector<CWorker *> workerList;
std::mutex workerListCS;

namespace {
  //anonymous namespace so these symbols are not going to be dlsym'ed by the initializer
...
NTAPI
#endif
ErrorThrow(jmp_buf *onError)
{
	//find the current thread in the workerlist and go to it's onError
	longjmp(*onError, 1);
	while (1)
	{
		OutputDebugStringA("ERRORLOOPING");
	}
}
...
LONG NTAPI ErrorFilter(struct _EXCEPTION_POINTERS* ExceptionInfo)
{
	...
	{
		//find the thread this error happened in

		DWORD tid = GetCurrentThreadId();
		int i;
		CWorker *thisworker = NULL;

		workerListCS.lock();
		for (i = 0; i < workerList.size(); i++)
		{
			if (workerList[i]->threadid == tid)
			{
				thisworker = workerList[i];
				break;
			}
		}
		workerListCS.unlock();

		if (thisworker && (thisworker->ExpectingAccessViolations))
		{
#ifdef _AMD64_
			ExceptionInfo->ContextRecord->Rip = (UINT_PTR)ErrorThrow;
			ExceptionInfo->ContextRecord->Rcx = (UINT_PTR)(&thisworker->onError); 		

			ExceptionInfo->ContextRecord->Rsp -= 32;

			*(PUINT64)(ExceptionInfo->ContextRecord->Rsp + 8) = ExceptionInfo->ContextRecord->Rcx;


#else
			ExceptionInfo->ContextRecord->Eip = (DWORD)ErrorThrow;
			ExceptionInfo->ContextRecord->Esp -= 8;  
			*(PDWORD)(ExceptionInfo->ContextRecord->Esp+4)=(DWORD)(&thisworker->onError); //[esp+4] as [esp] is the return address
...
void CPipeServer::CheckForShutdown(void)
{
  while (MDC_ShuttingDown==0)
  {
   // OutputDebugStringA("Boop");
    if ((mono_runtime_is_shutting_down) && (mono_runtime_is_shutting_down()))
    {
      ...
      Sleep(1000);
      OutputDebugStringA("Killing workers");
      workerListCS.lock();
      for (i = 0; i < workerList.size(); i++)
      {
        if (!workerList[i]->finished)
          workerList[i]->SetPipeHandle(0);
      }
      workerListCS.unlock();
      ...
      {
        OutputDebugStringA("Killing server pipe");
        CancelIoEx(pipehandle, NULL);
        DisconnectNamedPipe(pipehandle);
        CloseHandle(pipehandle);
        pipehandle = 0;
        OutputDebugStringA("Killed server pipe");
      }
#endif
...
void CPipeServer::CreatePipeAndSpawnWorkers(void)
{
	//HANDLE pipehandle;
	int c=0;


  std::thread t(&CPipeServer::CheckForShutdown, this);

	while (1)
	{
	  c++;
#ifndef _WINDOWS
	  OutputDebugString("CreatePipeAndSpawnWorkers iteration %d  (hMono=%x)",c, hMono);
#endif
	  
		if ((mono_runtime_is_shutting_down) && (mono_runtime_is_shutting_down()))
		{
			MDC_ShuttingDown = 1;
			OutputDebugString("MDC lib shutting down\n");

      
      if (attachedthreads.size())
      {
        OutputDebugStringA("There was still an unclosed thread");
      }
      t.join();
      ...
      exit(0); //cheating
		...
		pipehandle = CreateNamedPipeW(datapipename, PIPE_ACCESS_DUPLEX, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 100, 256 * 1024, 16, INFINITE, NULL);
		...
		if ((pipehandle == 0) || (pipehandle == INVALID_HANDLE_VALUE))
		{
			...
			MDC_ServerPipe = (HANDLE)0xdeadbeef; //indicates that it got read out and the next thread can go
		}
    BOOL r = ConnectNamedPipe(pipehandle, NULL);

    if ((r == FALSE) && (GetLastError() == ERROR_PIPE_CONNECTED))
      r = TRUE;

#else
    OutputDebugString("Calling linux version of CreateNamedPipe");
    OutputDebugString("yyy");
    BOOL r;
		pipehandle = ::CreateNamedPipe((char*)datapipename);

		OutputDebugString("pipehandle=%x",pipehandle);
		r=pipehandle != INVALID_HANDLE_VALUE;
#endif
      ...
      OutputDebugStringA("ConnectNamedPipe returned TRUE");
			...
			for (i = 0; i < workerList.size(); i++)
			{
				...
				{
					found = true;
					workerList[i]->SetPipeHandle(pipehandle);
					workerList[i]->Start(); //launches the thread that runs the commandloop									
					break;
				}
			}
			...
			if (!found)
			{
				CWorker *w = new CWorker(pipehandle);
				workerListCS.lock();
...
{
#ifndef _WINDOWS
  OutputDebugString("InitMono entry: hMono is %x\n", hMono);
#endif

	if (hMono)
	{
	  OutputDebugString("hMono is already set. Exiting InitMono\n");
	  return;
	}
	...
	if (fp==&mono_thread_attach)
	{
	  OutputDebugStringA("Found self. Not useful and shouldn't happen anymore");
	  fp = dlsym(RTLD_NEXT, "mono_thread_attach");
	}
	OutputDebugStringA("dlsym(RTLD_DEFAULT/NEXT, \"mono_thread_attach\") returned %p\n", fp);
	...
	else
	{
		...
		if (fp)
		{
			...
			OutputDebugStringA("have set hMono to %p", hMono);
		}
	}
	...
	if (fp == NULL)
	{
	  OutputDebugString("symbol not found. Scanning for modules instead\n");

		//find the path to libmono.so and load that
		char* path=NULL;
    if (monolibpath)
    {
      OutputDebugString("monolibpath is set to %s", monolibpath);
      path=strdup(monolibpath);
    }
    else
    {
      OutputDebugString("monolibpath is not set");

    }

		if (path==NULL)
		{
		  OutputDebugString("Scanning for libmono.so");

		  path=findModulePath((char*)"libmono.so");
		}

		//if (path==NULL)
		//{
		//OutputDebugString("Scanning for libmono*");
		//  path=findModulePath((char*)"libmono");
		//}
		...
		if (path)
		{
			...
			/*if (hMono==NULL)
			{
			  hMono = dlopen(path, RTLD_NOW | RTLD_NOLOAD| RTLD_GLOBAL);

			}
      if (hMono==NULL)
      {
        hMono = dlopen(path, RTLD_NOLOAD| RTLD_GLOBAL);

      }*/

			//hMono = dlopen(NULL, RTLD_NOW);
		...
		{
		  OutputDebugString("libmono.so not found\n");
	...
	if ((!hMono) && (monolibpath))
	{
    hMono=GetModuleHandleA(monolibpath);
    if (!hMono)
      hMono=LoadLibraryA(monolibpath);
	}
	...
	if (hMono)
	{
	  OutputDebugString("hMono is valid\n");
		...
		{
		  OutputDebugString("hasRealMonoHandle == 0\n");
			hMono = RTLD_DEFAULT;
		}
		else
		{
		  OutputDebugString("hasRealMonoHandle\n");
		}


		if (GetProcAddress(hMono, "il2cpp_thread_attach"))
		{
		  OutputDebugString("il2cpp_thread_attach found in the module. It's il2pp");
		  il2cpp=TRUE;
		}
		else
		{
		  OutputDebugString("No il2cpp_thread_attach found in the module. Likely normal mono");

		  if (GetProcAddress(hMono, "mono_thread_attach"))
		  {
		    OutputDebugString("Confirmed. mono_thread_attach is present. It is libMono");
		  }
		  else
		  {
		    OutputDebugString("Unexpected result. Neither il2cpp_thread_attach nor mono_thread_attach are present in the module");
		    hMono=NULL;
		...
		if (il2cpp)
		{
		  OutputDebugString("it is il2cpp\n");
      ...
      il2cpp_class_get_static_field_data = (IL2CPP_CLASS_GET_STATIC_FIELD_DATA)GetProcAddress(hMono, "il2cpp_class_get_static_field_data");
			...
			if (mono_domain_get)
			{
			  OutputDebugString("calling il2cpp_domain_get");
#ifndef _WINDOWS
			  OutputDebugString("il2cpp_domain_get is located at %p\n",mono_domain_get);
#endif
			  domain = mono_domain_get();

			  OutputDebugString("after calling il2cpp_domain_get");
#ifndef _WINDOWS
			  OutputDebugString("domain=%p",domain);
#endif
			}
			else
			  OutputDebugString("mono_domain_get==NULL");
	...
	{
	  OutputDebugString("hModule not valid\n");
	}

	OutputDebugString("InitMono() end reached\n");
}


CWorker::CWorker(HANDLE p)
{
...
{
  DetachThreadFromMonoRuntime();

}

char* CWorker::ReadString(void)
{
...
void CWorker::WriteString1(const char* value) //for 1 byte length strings
{
	if (value)
	{
...
void CWorker::WriteString(const char* value)
{
	if (value)
	{
		int n = (int)strlen(value);
...
void CWorker::FreeString(char* value)
{
	if (value)
		free(value);
}
...
void CWorker::SetMonoLib()
{
  OutputDebugString("SetMonoLib");
  if (monolibpath)
  {
    free(monolibpath);
    monolibpath=NULL;
  ...
  monolibpath=ReadString();
#ifndef _WINDOWS
  OutputDebugString("SetMonoLib to %s", monolibpath);
#endif
  InitMono();
  ConnectThreadToMonoRuntime();

  WriteByte(mono_thread_attach!=0);
}



void CWorker::DetachThreadFromMonoRuntime()
{


  if (mono_selfthread)
  {
    attachedthreads.erase(mono_selfthread);
...
void CWorker::ConnectThreadToMonoRuntime()
{
  DetachThreadFromMonoRuntime();
  ...
  if ((mono_runtime_is_shutting_down) && (mono_runtime_is_shutting_down()))
  {
    ...
    attachedthreads[mono_selfthread] = 1;
	}

	attached = mono_selfthread != NULL;
}

void CWorker::Object_New()
{
	void* domain;
	if (mono_get_root_domain)
...
void CWorker::Object_GetClass()
{
	...
	try
	{
		...
		if (classname)
			classnameLength=(int)strlen(classname);
		else
		{
			WriteQword(0);
			return;
		...
		if (klass != 0)
		{
			...
			WriteWord((WORD)classnameLength);
			Write(classname, classnameLength);
		}
		else
		{
			WriteQword(0);
...
void CWorker::SetCurrentDomain(void)
{
...
void CWorker::GetImageFromAssembly()
{
...
void CWorker::EnumClassesInImageEx()
{
...
void CWorker::EnumClassesInImage()
{
	int i;
	void* image = (void*)ReadQword();
	if (image == NULL)
...
void CWorker::EnumFieldsInClass()
{
...
void CWorker::EnumImplementedInterfacesOfClass()
{
	void* klass = (void*)ReadQword();
	void* iter = NULL;
	void* interfaces;
...
void CWorker::EnumMethodsInClasses()
{
  OutputDebugStringA("EnumMethodsInClasses");
  int classcount=ReadDword();

#ifndef _WINDOWS
  OutputDebugStringA("classcount=%d\n", classcount);
#endif
  QWORD *classes=(QWORD*)malloc(sizeof(QWORD)*classcount);
  int i;


  Read(classes, sizeof(QWORD)*classcount);

  OutputDebugStringA("Classes read");

  CMemStream reply;
  //for each class get the methods and end it with a 0
...
#ifndef _WINDOWS
    OutputDebugStringA("Class %d:",i);
#endif

    do
    {
      if (reply.GetSize()>32*1024*1024) //32MB seems big enough
      {
        OutputDebugStringA("EnumMethodsInClasses result list too big");
        reply.WriteQword(1); //
        break;
      }


      method = mono_class_get_methods((void*)(classes[i]), &iter);
      reply.WriteQword((UINT_PTR)method);
      ...
      if (method)
      {
        ...
        reply.WriteWord((WORD)sName.size());
        reply.Write((PVOID)sName.c_str(), (WORD)sName.size());

        reply.WriteDword(flags);

        if (il2cpp)
          reply.WriteQword(*(uint64_t*)method); //first bytes of the method. useful for il2cpp
      }
  ...
  OutputDebugStringA("freeing classlist\n");

  free(classes);



  OutputDebugStringA("writing result\n");
  WriteDword(reply.GetSize());
  Write(reply.GetMemory(), reply.GetSize());
...
void CWorker::EnumMethodsInClass()
{
	void* c = (void*)ReadQword();
	void* iter = NULL;
	void* method;
...
void CWorker::GetMethodFullName()
{
	void* method = (void*)ReadQword();
...
void CWorker::GetClassNamespace()
{
...
void CWorker::FreeMethod()
{
	if (mono_free_method)
		mono_free_method((void*)ReadQword());
}

void CWorker::FreeObject()
{

	//not yet implemented
}

void CWorker::GetMonoDataCollectorVersion()
{
  OutputDebugString("GetMonoDataCollectorVersion");
...
void CWorker::GetClassFromPointer()
{
...
void CWorker::GetTypeFromPointerType()
{
...
void CWorker::DisassembleMethod()
{
	void* method = (void*)ReadQword();
	if (!il2cpp)
	{
...
void CWorker::GetMethodParameters()
{
	void* method = (void*)ReadQword();
	int i;
...
void CWorker::GetMethodSignature()
{
	void* method = (void*)ReadQword();
	int i;
...
void CWorker::GetParentClass(void)
{
	void* klass = (void*)ReadQword();
	UINT_PTR parent = 0;
	if (klass)
...
void CWorker::GetClassNestedTypes(void)
{
  int32_t count;
//	INT32 count = 0;
	void* klass = (void*)ReadQword();
...
void CWorker::GetClassNestingType(void)
{
...
void CWorker::GetClassImage(void)
{
	UINT_PTR image = 0;
	void* klass = (void*)ReadQword();
	if (klass)
...
void CWorker::GetClassType()
{
	void* kls = (void*)ReadQword();
	WriteQword((UINT64)mono_class_get_type(kls));
}

void CWorker::GetClassOfType()
{
	void* type = (void*)ReadQword();
	WriteQword((UINT64)mono_type_get_class(type));
}

void CWorker::GetTypeOfMonoType()
{
	void* type = (void*)ReadQword();
	if (type)
		WriteDword((UINT64)mono_type_get_type(type));
	else
		throw("GetTypeOfMonoType: Invalid type");
}

void CWorker::GetReflectionTypeOfClassType()
{
...
void CWorker::GetReflectionMethodOfMethod()
{
...
void CWorker::GetVTableFromClass(void)
{
	void* domain = (void*)ReadQword();
	void* klass = (void*)ReadQword();
...
void CWorker::GetStaticFieldAddressFromClass(void)
{
	void* domain = (void*)ReadQword();
	void* klass = (void*)ReadQword();
...
void CWorker::GetFieldClass(void)
{
...
void CWorker::GetArrayElementClass(void)
{
	void* klass = (void*)ReadQword();
...
void CWorker::FindMethodByDesc(void)
{
	void* image = (void*)ReadQword();
...
void CWorker::InvokeMethod(void)
{
...
void CWorker::LoadAssemblyFromFile(void)
{
...
std::string CWorker::GetFullTypeNameStr(void* klass, char isKlass, int nameformat)
...
void CWorker::GetFullTypeName(void)
{
...
void CWorker::IsTypeByReference()
{
...
void CWorker::GetArrayElementSize()
{
	void* klass = (void*)ReadQword();
...
void CWorker::IsIL2CPP()
{
	WriteByte(il2cpp);
}

void CWorker::FillOptionalFunctionList()
{
...
void CWorker::GetStaticFieldValue()
{
  ...
  val.b = 0;
	if (il2cpp)
	{
		if (il2cpp_field_static_get_value && mono_field_get_type && il2cpp_class_from_type && mono_class_is_valuetype && mono_class_instance_size)
		{
        void *fieldType=mono_field_get_type(Field);
        if (fieldType)
        {
          void* c=il2cpp_class_from_type(fieldType);
          if (c)
          {
            if (mono_class_is_valuetype(c) == 0)
            {
              il2cpp_field_static_get_value(Field, &val);
            }
            else
            {
              int s = mono_class_instance_size(c);
		...
		if (mono_field_static_get_value)
		{
			if (mono_field_get_flags)
			{
				...
				if (flags & 0x10) //0x10=FIELD_ATTRIBUTE_STATIC
				{
					...
					{
					NOTENUM:
						if ((mono_vtable_get_static_field_data) && (Vtable))
						{
...
void CWorker::SetStaticFieldValue()
{
	void* Vtable = (void*)ReadQword();
	void* Field = (void*)ReadQword();
	QWORD val = ReadQword();
...
void CWorker::CommandLoop(void)
{
  ...
  debug_status=6;
	...
	OutputDebugString("CWorker::CommandLoop\n");
	...
	//if (attached)
	{
		...
		while (1)
		{
			...
			try
			{
				...
				while (TRUE)
				{
				 // OutputDebugString("Waiting for command\n");
					command = ReadByte();

        //  char cs[25];
         // snprintf(cs, 25, "command=%d", command);
         ...
         // OutputDebugStringA(cs);
					...
					{
						MDC_ShuttingDown = 1;						
						throw("Message loop: Mono shutting down");
					}
					...
					case MONOCMD_ISMONOLOADED:
						if ((mono_runtime_is_shutting_down) && (mono_runtime_is_shutting_down()))
						{
						  ...
						  WriteByte(monoIsValid || (hMono!=0)); //hMono could be the default handle (0)

						break;

					case MONOCMD_SETMONOLIB:
					  //sets the mono lib and initializes mono with this (mainly needed for linux when the libmono can't be found. CE will provide it)
					  SetMonoLib();
					...
					case MONOCMD_ENUMMETHODSINCLASSES:
					  OutputDebugStringA("EnumMethodsInClasses");
					  EnumMethodsInClasses();
					  break;
...
void CWorker::workerThreadEntry()
{
	try
	{
		//attach to mono (if possible)
		ConnectThreadToMonoRuntime();
		CommandLoop();
...
DWORD WINAPI workerThreadEntryInvoker(LPVOID worker)
{
	CWorker *w = (CWorker*)worker;

	OutputDebugString("Hello from worker entry\n");
  ...
  OutputDebugStringA("worker finished");
...
void CWorker::SetPipeHandle(HANDLE newPipeHandle)
{
  if ((pipehandle) && (pipehandle != INVALID_HANDLE_VALUE))
  {
    CancelIoEx(pipehandle, NULL);
...
void CWorker::Start(void)
{
	finished = false;


#ifdef _WINDOWS
  HANDLE th=CreateThread(NULL, 0, workerThreadEntryInvoker, this, 0, &(this->threadid));
  CloseHandle(th);
#else
	pthread_t pt;
	OutputDebugString("Creating worker thread\n");
	pthread_attr_t attr;
  pthread_attr_init(&attr);
  pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

	pthread_create(&pt, &attr, (void* (*)(void*)) workerThreadEntryInvoker,  this);
	OutputDebugString("After creating worker thread\n");
